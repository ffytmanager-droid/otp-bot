...
class DatabaseManager {
  ...
  async getMonthlyDeposit(userId) {
    ...
    return result.rows[0]?.total_deposit || 0;
  }
  ...
  async transferBalance(fromUserId, toUserId, amount, note = '') {
    ...
    try {
      ...
      await client.query(
        'INSERT INTO balance_transfers (from_user_id, to_user_id, amount, note) VALUES ($1, $2, $3, $4)',
  ...
  async getUsersForVerification() {
    const result = await this.client.query(
      'SELECT user_id FROM users WHERE channel_joined = TRUE'
    );
  ...
  async checkDuplicateUTR(utr) {
    const result = await this.client.query(
      'SELECT id FROM topup_requests WHERE utr = $1',
      [utr]
    );
    return result.rows.length > 0;
  }
    ...
    await this.client.query(
      `INSERT INTO referrals (referrer_id, referred_id, referral_code)
       ...
       ON CONFLICT (referred_id) DO UPDATE SET
         referrer_id = $1, referral_code = $3, joined_at = CURRENT_TIMESTAMP, is_active = TRUE`,
